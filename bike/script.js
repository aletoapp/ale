...
/* ── YOUTUBE LAZY LOAD (iframe só criado no clique) ── */
...
if (ytWrapper) {
  ...
  function loadYouTube() {
    // Cria o iframe apenas no momento do clique (lazy YouTube)
    const iframe = document.createElement('iframe');
    iframe.src = 'https://www.youtube.com/embed/uD67_gYFRjQ?rel=0&autoplay=1';
    iframe.title = 'R10 Full — Bike Elétrica 1000W';
    ...
    // Remove listeners após primeiro clique
    ytWrapper.removeEventListener('click', loadYouTube);
    ytWrapper.removeEventListener('keydown', ytKeyHandler);
  }

  function ytKeyHandler(e) {
  ...
  ytWrapper.addEventListener('click', loadYouTube);
  ytWrapper.addEventListener('keydown', ytKeyHandler);
}

/* ── MODAL VÍDEO ── */
const bikeVideo = document.getElementById('bike-video');
...
function openVideoModal() {
  // Pausa o vídeo de fundo para economizar recursos
  if (bikeVideo) bikeVideo.pause();
...
/* ── BUG FIX: usar closest() para capturar o trigger correto mesmo ao clicar na <img> interna ── */
